# 3
index="crossUS4"
#echo "first part begin:"$(date) >> /data/local/tmp/Timetest_back.log
...
# echo "Tmobile After curl" >> /data/local/tmp/Timetest_back.log
cat /data/local/tmp/${SIMcard}_google.txt | egrep -o "https:.*googlevideo.com" >> /data/local/tmp/${SIMcard}_google_dns.txt
# echo "Tmobile After extract" >> /data/local/tmp/Timetest_back.log
# echo $dns
# Get DNS name and resolve its IP address.
...
echo "First part scamper Google end:"$(date) >> /data/local/tmp/Timetest_${index}.log
